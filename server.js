...
try {
  ...
  console.error('ERROR initializing Groq:', err.message);
}
...
app.post('/api/generate', async (req, res) => {
  try {
    if (!groq) {
      return res.status(500).json({ error: 'Groq API not initialized - GROQ_API_KEY may be missing or invalid' });
    }
...
async function sendScriptEmail(scriptRecord) {
  try {
    if (!resend || !process.env.RECIPIENT_EMAIL) {
      console.log('Resend not configured or RECIPIENT_EMAIL missing. Skipping email.');
      return;
    }
...
async function generateScripts(storyId) {
  if (!groq) {
    throw new Error('Groq API not initialized - GROQ_API_KEY may be missing or invalid');
  }
